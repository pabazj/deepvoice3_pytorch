module.exports = {
   unicodeToSinglish: function(unicodeText) {
    return _unicodeToSinglish(unicodeText);
}
}

function _unicodeToSinglish(unicodeText){
	var text;
    var nVowels;
    ...
    vowelsUni[25]='අ';    vowels[25]='a';    vowelModifiersUni[25]='';
    nVowels=26;
    ...
    var s,r,v,debug_text = "";
    text = unicodeText;  

    //vowels
    for (var i=0; i<vowelsUni.length; i++){
        r = new RegExp(vowelsUni[i], "g");
        text = text.replace(r, vowels[i]);
    }

    //consonents + HAL
    for (var i=0; i<consonantsUni.length; i++){
        r = new RegExp(consonantsUni[i] + "්", "g");
        text = text.replace(r, consonants[i]);
    }
